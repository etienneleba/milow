...
export default class FunctionCaller {
  constructor(private readonly functionResolver: FunctionResolver) {}
  ...
  async call(functionCalls: FunctionCall[]): Promise<FunctionResult[]> {
    ...
    for (const functionCall of functionCalls) {
      ...
      const functionResult = await functionCallable.call(
        functionCall.parameters,
      );
      functionResults.push(new FunctionResult(functionCall.id, functionResult));
    }
