...
export default class FileConfigProvider implements ConfigProvider {
  ...
  get(): Config {
    ...
    if (existsSync(this.configFilePath)) {
      ...
      outroError(
        `Config file milow.config.json does not exist. Run ${chalk.green("milow init")} to create the config file`,
      );
      process.exit();
    }
    ...
    if (!configContent) {
      outroError(
        `Config file milow.config.json is empty.Delete it and run ${chalk.green("milow init")} to create the config file`,
      );
      process.exit();
    }
    ...
    if (!modelNames.includes(configObject.model)) {
      outroError(
        `The model ${chalk.green(configObject.model)} is not supported yet by Milow. Check all the supported models with the command ${chalk.green("milow check models")}. Feel free to contribute to the project to add new models 🚀`,
      );
      process.exit();
    }

    const modelProviderAPIKeyName = modelResolver
      .resolve(configObject.model)
      .getAPIKeyName();
    ...
    if (!apiKey) {
      outroError(
        `Please expose your api key. ${chalk.green("export " + modelProviderAPIKeyName + "=[your api key]")}`,
      );
      process.exit();
    }
  ...
  private checkPropertiesExist(configObject: object) {
    ...
    for (const configProperty of Object.values(CONFIG_PROPERTIES)) {
      if (
        !configObject.hasOwnProperty(configProperty) ||
        !configObject[configProperty]
      ) {
    ...
    if (errors.length > 0) {
      for (const error of errors) {
        log.error(
          `The property ${error} is not defined or is empty in your config file. Check your config file and update it`,
        );
      }
