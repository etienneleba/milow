...
export const initCommand = command(
  ...
  async (argv) => {
    ...
    if (!fs.existsSync(configFilePath)) {
      ...
      log.step(
        "The milow.config.json file has been created, take the time to specify the right value for your project\n",
      );
    } else {
      log.step(
        "The milow.config.json file already exist, next step is milow run",
      );
    }
